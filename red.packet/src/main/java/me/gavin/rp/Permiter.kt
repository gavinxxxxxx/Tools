...
inline fun <reified T : AccessibilityService> TileService.doIfPermissionGrant4Accessibility(block: () -> Unit) {
    if (checkPermission4Accessibility<T>() && isServiceRunning<T>()) {
        block.invoke()
    } else {
        "该功能需要开启辅助功能权限".toast()
        startActivityAndCollapse(Intent(Settings.ACTION_ACCESSIBILITY_SETTINGS).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK))
    }
}
...
inline fun <reified T : AccessibilityService> Context.checkPermission4Accessibility(): Boolean {
    ...
    return runCatching {
        Settings.Secure.getInt(contentResolver, Settings.Secure.ACCESSIBILITY_ENABLED, 0)
                ...
                ?: false
    }.getOrDefault(false) && isServiceRunning<T>()
}


fun TileService.doIfPermissionGrant4Notification(channelId: String? = null, block: () -> Unit) {
...
fun Context.checkPermission4Notification(channelId: String? = null): Boolean {
    return NotificationManagerCompat.from(this).areNotificationsEnabled()
}

@SuppressLint("InlinedApi")
fun TileService.doIfPermissionGrant4NotificationListener(block: () -> Unit) {
    if (checkPermission4NotificationListener()) {
        block.invoke()
    } else {
        "该功能需要通知监听权限".toast()
        startActivityAndCollapse(Intent(Settings.ACTION_NOTIFICATION_LISTENER_SETTINGS).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK))
    }
}
